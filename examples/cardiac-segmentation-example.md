...
### 数据集全面分析
生成 `dataset_analysis_report.md` + 可视化图片：

- 训练集：800 例，验证集：200 例
- 格式：NIfTI (.nii.gz)，维度 (1, 256, 256)
- 类别：3 类（左心室、右心室、心肌）
- 类别不平衡：心肌像素占比 ~5%，不平衡比 12:1
- 数据质量：无损坏文件，无重复样本
- 像素值范围：0-2048（HU 值），需归一化
- PCA 可视化：3 类有一定分离度，心肌与背景有重叠

**调参建议**（提取到 decision_log.md）：
1. 使用 CE+Dice 组合 loss（心肌像素占比 < 1%）
2. 数据增强加入 elastic 变形（心脏形状变化大）
3. 像素值归一化到 0-1
