...
根据 `state.json` 中的 `autonomy_mode` 决定行为：

| 模式 | 行为 |
|------|------|
| `full` | 默认使用**快速**分析。仅当用户明确要求深入分析时才用**全面**。 |
| `interactive` | 给用户两个选项（不设超时，等待用户选择）。默认建议"快速"，用户明确要求深入分析时才用"全面"。 |
...
然后按 `autonomy_mode` 选择数据集策略：

| 模式 | 行为 |
|------|------|
| `full` | 默认使用**选项 A**：先用 10%-20% 子集快速调参找方向，最后 2-3 轮全量验证。若用户在对话中已明确要求全量，则直接用全量。 |
| `interactive` | 给用户两个选项（不设超时，等待用户选择）：<br>• **选项 A**：先用 10%-20% 子集快速调参找方向，最后 2-3 轮全量验证（快，省资源，但最终指标可能有偏差）<br>• **选项 B**：直接用全量数据集（结果可靠，但慢，耗资源） |

无论哪种模式，都需在 `decision_log.md` 中记录选择理由。
